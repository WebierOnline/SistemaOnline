...
If Len(Trim(txtQuantNova.Text)) = 0 Or Not IsNumeric(txtQuantNova.Text) Then
   MsgBox "Digite uma quantidade válida.", vbExclamation, "Quantidade inválida"
   txtQuantNova.SetFocus
   Exit Sub
End If
dNova = Val(Replace(Replace(txtQuantNova.Text, ".", ""), ",", "."))
If dNova <= 0 Then
   MsgBox "A quantidade deve ser maior que zero.", vbExclamation, "Quantidade inválida"
   txtQuantNova.SetFocus
   Exit Sub
End If
...
sSQL = "INSERT INTO Produtos_Quant (Codigo, COD_PRODUTO, Data, COD_ENTRADA, FORMA, QUANT, TIPO, HORA, COD_USUARIO, ESTOQUE) VALUES (" & _
   AutoNumeracao & ", " & txtCodProduto.Text & ", CONVERT(DATETIME, '" & Format(Date, ocDATA) & "', 103), 0, 'AJUSTE', " & Replace(CDbl(txtQuantNova.Text), ",", ".") & ", 'ADIÇÃO', '" & Format(Now, ocHRMN) & "', " & txtCodUsuario.Text & ", " & Replace(CStr(dAtual + dNova), ",", ".") & ");"
dbData.Execute sSQL
...
If dNova > dAtual Then
   MsgBox "Quantidade a remover (" & txtQuantNova.Text & ") é maior que o estoque atual (" & txtQuant.Text & ").", vbExclamation, "Estoque insuficiente"
   txtQuantNova.SetFocus
   Exit Sub
End If
...
sSQL = "INSERT INTO Produtos_Quant (Codigo, COD_PRODUTO, Data, COD_ENTRADA, FORMA, QUANT, TIPO, HORA, COD_USUARIO, ESTOQUE) VALUES (" & _
   AutoNumeracao & ", " & txtCodProduto.Text & ", CONVERT(DATETIME, '" & Format(Date, ocDATA) & "', 103), 0, 'AJUSTE', " & Replace(CDbl(txtQuantNova.Text), ",", ".") & ", 'REMOÇÃO', '" & Format(Now, ocHRMN) & "', " & txtCodUsuario.Text & ", " & Replace(CStr(dAtual - dNova), ",", ".") & ");"
dbData.Execute sSQL
...
If dExato < 0 Then
   MsgBox "A quantidade não pode ser negativa.", vbExclamation, "Quantidade inválida"
   txtQuantNova.SetFocus
   Exit Sub
End If
dAtual = Val(Replace(Replace(txtQuant.Text, ".", ""), ",", "."))
...
If dDiff = 0 Then
   MsgBox "O estoque já está em " & txtQuantNova.Text & ". Nenhuma alteração necessária.", vbInformation, "Exato"
   Exit Sub
End If
