# -*- coding: utf-8 -*-
"""Cria OnlineCommerce/Forms/frmIBPTProgresso.frm (cp1252/ASCII, CRLF) - forminha
   modeless de progresso pra atualizacao da tabela IBPT pela nuvem."""
...
data = frm.replace("\r\n", "\n").replace("\n", "\r\n").encode("cp1252")
p = r"C:\projeto\OnlineCommerce\Forms\frmIBPTProgresso.frm"
with open(p, "wb") as f:
    f.write(data)
print("escrito", p, len(data), "bytes")

# adiciona no .vbp logo depois do frmImportarIBPT.frm
vbp = r"C:\projeto\OnlineCommerce\OnlineCommerce.vbp"
b = open(vbp, "rb").read().replace(b"\r\n", b"\n")
anchor = b"Form=Forms\\frmImportarIBPT.frm\n"
assert b.count(anchor) == 1, b.count(anchor)
if b"Form=Forms\\frmIBPTProgresso.frm\n" not in b:
    b = b.replace(anchor, anchor + b"Form=Forms\\frmIBPTProgresso.frm\n", 1)
b = b.replace(b"\n", b"\r\n")
open(vbp, "wb").write(b)
print("vbp atualizado")
