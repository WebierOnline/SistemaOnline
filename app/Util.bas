...
'envia um mensagem para o windows
Declare Sub keybd_event Lib "user32" (ByVal bVk As Byte, ByVal bScan As Byte, ByVal dwFlags As Long, ByVal dwExtraInfo As Long)

'constantes para BrowseFolder
...
Const API_KEY_WPP = "26f4c100ffec076234198d8c10e85b845839b3f40340b2a212f23cf137c13f7e"
Public Const urlAPIWhatsBase = "https://whatsapi.lotesis.cloud/"
Public API_WHATS_MENSAGEM_ERRO As String

Public xWinRar As String, xWinZip As String
'tirei o mensagemErro As String
Public Sub ZapConfigurar(ApiEmail As String, ApiToken As String, ApiIdApp As String)
Dim iretorno As Boolean, mensagemRetorno As String, mensagemErro As String
Dim ekZap As zapzap.cZap
    Set ekZap = New zapzap.cZap
    
    iretorno = ekZap.ConfigurarDLL(ApiEmail, ApiToken, ApiIdApp, eProvedor_EkklesiaSoft, mensagemErro)
    'iretorno = ekZap.configurarDLL(ApiEmail, ApiToken, ApiIdApp, eProvedor_Solutek, mensagemErro)
    
    If mensagemErro <> "" Then
       MsgBox "ERRO: " & mensagemErro, vbCritical + vbOKOnly
    Else
       MsgBox "DLL Configurada com sucesso!", vbInformation + vbOKOnly
    End If
...
Dim ekZap As zapzap.cZap
    ...
    iretorno = ekZap.Saldo(mensagemRetorno, mensagemErro)
...
Dim ekZap As zapzap.cZap
    ...
    iretorno = ekZap.Status(mensagemRetorno, mensagemErro)
...
Public Function ZapConectado() As Boolean
Dim iretorno As Boolean, mensagemRetorno As String, mensagemErro As String
Dim ekZap As New zapzap.cZap
...
Public Sub ZapConectar()
Dim iretorno As Boolean, mensagemRetorno As String, mensagemErro As String
Dim ekZap As zapzap.cZap

    Set ekZap = New zapzap.cZap
    
    iretorno = ekZap.lerQRCode(mensagemRetorno, mensagemErro)
...
Public Function ValidateNull2(ByVal Valor As String, ByVal Var As ADODB.Field) As Variant
   Select Case Var.Type
      Case adBigInt, adCurrency, adDecimal, adDouble, adInteger, adNumeric, adSingle, adSmallInt, adTinyInt
         ValidateNull2 = IIf(IsNull(Valor), 0, Valor)
      Case adUnsignedBigInt, adUnsignedInt, adUnsignedSmallInt, adUnsignedTinyInt
         ValidateNull2 = IIf(IsNull(Valor), 0, Valor)
      Case adDate, adDBDate, adDBTime, adDBTimeStamp
         ValidateNull2 = IIf(IsNull(Valor), 0, Valor)
      Case adBoolean
         ValidateNull2 = IIf(IsNull(Valor), False, Valor)
      Case adBSTR, adChar, adLongVarBinary, adLongVarChar, adLongVarWChar, adVarChar, adVarWChar, adWChar
         ValidateNull2 = IIf(IsNull(Valor), "", Valor)
      Case Else
         ValidateNull2 = IIf(IsNull(Valor), "", Valor)
   End Select
End Function
...
Public Function CompactarBackup(nArqBackup As String) As String
Dim FileZipName As String, PathToCompress As String, DestPath As String, FullPathZip As String, NomeEmp As String
Dim sSQL As String, r As ADODB.Recordset
    ...
    FileZipName = NomeEmp & "_" & Retira(r("cnpj"), "./- ", UM_A_UM) & ".rar"
    
    'Caminho + ficheiro.rar
    FullPathZip = Transforma(oCfg.Value & IIf(Right(oCfg.Value, 1) = "\", "", "\") & FileZipName)
    
    'Caminho a comprimir (nota : no final não deixar no fim da directoria o simbolo de '\'
    DestPath = IIf(Right(oCfg.Value, 1) = "\", Left(oCfg.Value, Len(oCfg.Value) - 1), oCfg.Value)
    PathToCompress = Transforma(DestPath)
    
    'Chama o compressor que se encontra instalado para o efeito.
    If xWinRar <> "" Then
        Shell xWinRar & " a -d -ep " & FullPathZip & " " & PathToCompress, vbNormalFocus ', vbHide
    Else
        Shell xWinZip & " -a " & FullPathZip & " " & PathToCompress, vbNormalFocus ', vbHide
    End If
    
    CompactarBackup$ = FullPathZip
...
Public Function Moeda(din As TextBox)
    din.Text = Format(din.Text, "##,##0.00")
End Function

Public Sub Limpa_Tudo(f As Form)
Dim i As Integer
On Error Resume Next
...
For i = 0 To f.Controls.Count - 1
    If TypeOf f.Controls(i) Is ComboBox Then
       f.Controls(i).Text = ""
    End If
Next i

For i = 0 To f.Controls.Count - 1
    If TypeOf f.Controls(i) Is CheckBox Then
       f.Controls(i).Value = False
    End If
Next i

For i = 0 To f.Controls.Count - 1
    If TypeOf f.Controls(i) Is OptionButton Then
       f.Controls(i).Value = 0
    End If
Next i

For i = 0 To f.Controls.Count - 1
    If TypeOf f.Controls(i) Is MaskEdBox Then
       f.Controls(i).Mask = ""
       f.Controls(i).Text = ""
    End If
Next i
...
Public Function SQLExecutaTratandoErro(pComandoSQL As String, Optional ByRef pnRegs As Long, Optional pMensagem As String = "Falha na execução de comando SQL!", Optional pMensagemTitulo As String = "SQL Executa") As Boolean
    Dim e As String
    'Executa o camando SQL
    e$ = SQLExecuta(pComandoSQL, pnRegs)
    'Se tem erro, entao exibe
    If Not Vazio(e$) Then MsgBox pMensagem & vbNewLine & e$, vbCritical, pMensagemTitulo
    'Faz retorno
    SQLExecutaTratandoErro = Not Vazio(e$)
End Function
...
Public Function BrowseFolderCallback(ByVal hwnd As Long, _
...
'Retorna o ponteiro do endereço de uma função para callback
'exemplo: lpCallBack = GetCallBackPointer(AddressOf NomeFuncaoCallBack)
Public Function GetCallBackPointer(lPointer As Long) As Long
 GetCallBackPointer = lPointer
End Function

Public Function VerificaCFOPEntrada(UFEmpresa As String, UFFornecedor As String, ProdutoST As String, Optional CFOP As Variant) As String
Dim xArg As String, icmsCST As String
 If UFEmpresa = UFFornecedor Then
    xArg = "1"
 Else
    xArg = "2"
 End If
 If Len(ProdutoST) = 3 Then
    icmsCST = Right(ProdutoST, 2)
 Else
    icmsCST = Right(ProdutoST, 3)
 End If
 If Not Vazio(CFOP) Then
    xArg = xArg & Right(CFOP, 3)
 Else
    Select Case icmsCST
     Case "II", "41", "400"
      xArg = xArg & "102"
     ...
     Case "NN", "40", "300"
      xArg = xArg & "102"
     Case "Alíquota", "00", "20", "101", "102", "201", "202"
      xArg = xArg & "102"
     Case "90", "900"
      xArg = xArg & "102"
     Case Else
...
Public Function EncodeBase64Arquivo(ByVal caminhoArquivo As String) As String
    Dim bBytes() As Byte
    ...
    iFile = FreeFile
    Open caminhoArquivo For Binary As #iFile
    ReDim bBytes(LOF(iFile) - 1)
    Get #iFile, , bBytes
    Close #iFile
    ...
    oNode.nodeTypedValue = bBytes

    'EncodeBase64Arquivo = Replace(oNode.Text, vbCrLf, "")
    
    Dim base64Raw As String
    base64Raw = oNode.Text
    base64Raw = Replace(base64Raw, Chr(13), "")
    base64Raw = Replace(base64Raw, Chr(10), "")
    EncodeBase64Arquivo = base64Raw
...
Public Function EncodeBase64(ByVal sText As String) As String
    ...
    ReDim bBytes(LenB(sText) - 1)
    CopyMemory bBytes(0), ByVal StrPtr(sText), LenB(sText)

    oNode.nodeTypedValue = bBytes

    ' - Remove quebras de linha inseridas automaticamente
    EncodeBase64 = Replace(oNode.Text, vbCrLf, "")
...
Public Function WhatsAppConectado() As Boolean
  Dim contentType As String, urlBase As String, url As String, ComandoSQL As String, jsonAPI As String, xCNPJCPF As String
  Dim qrCode As String, WPP_KEY As String, mensagemErro As String
  Dim parsedJsonObject As Object, Message As Variant
  Dim ekZap As New zapzap.cZap, iretorno As Boolean
  
    On Error GoTo deuErro
  
    Set ekZap = New zapzap.cZap
  
    API_WHATS_MENSAGEM_ERRO = ""
    
    contentType = "application/json;charset=utf-8"
    
    Dim obj As MSXML2.ServerXMLHTTP30
    Set obj = New MSXML2.ServerXMLHTTP30
    
    ComandoSQL = "SELECT CNPJ, WhatsAppApiKey " & _
                 "FROM Empresa"
    
    xCNPJCPF = Replace$(SQLExecutaRetorno(ComandoSQL, "CNPJ", ""), ".", "")
    xCNPJCPF = Replace$(xCNPJCPF, "/", "")
    xCNPJCPF = Replace$(xCNPJCPF, "-", "")
    
    WPP_KEY = SQLExecutaRetorno(ComandoSQL, "WhatsAppApiKey", "")
    
    If Vazio(WPP_KEY) Then
       WPP_KEY = "onlineinfo@ekklesiasoft.com.br"
       ComandoSQL = "UPDATE empresa SET WhatsAppApiKey = '" & WPP_KEY & "'"
       SQLExecuta ComandoSQL
    End If
    
    url = urlAPIWhatsBase & "instance/connectionState/" & WPP_KEY
    obj.Open "GET", url
    obj.setRequestHeader "Content-Type", contentType
    obj.setRequestHeader "apikey", API_KEY_WPP
    obj.send jsonAPI
    Dim resposta As String
    resposta = obj.responseText
    
    Set parsedJsonObject = JSON.parseJSON(resposta)
    
    If obj.Status = 200 Then
        If parsedJsonObject.Item("instance").Item("state") = "open" Then
           WhatsAppConectado = True
           Exit Function
'        Else
'           WhatsAppConectado = False
'           If FormEstaAberto("frmQRCode") Then Unload frmQRCode
'           Exit Function
        End If
    Else
        WhatsAppConectado = False
        ...
        For Each Message In parsedJsonObject.Item("response")
           mensagemErro = JSON.parseJSON(resposta).Item("response").Item("message").Item(1) + vbNewLine
        Next
        
        If Not Vazio(mensagemErro) Then
           'MsgBox mensagemErro, vbCritical + vbOKOnly
           API_WHATS_MENSAGEM_ERRO = mensagemErro
        Else
           'MsgBox parsedJsonObject.Item("error"), vbCritical + vbOKOnly
           API_WHATS_MENSAGEM_ERRO = parsedJsonObject.Item("error")
        End If
        If FormEstaAberto("frmQRCode") Then Unload frmQRCode
        Exit Function
    End If
...
deuErro:
    MsgBox Err.Description, vbCritical + vbOKOnly, "ERRO: WhatsAppConectado"
    Err.Clear
End Function

Public Function WhatsAppConectar() As Boolean
  Dim contentType As String, urlBase As String, url As String, ComandoSQL As String, jsonAPI As String, xCNPJCPF As String
  Dim qrCode As String, WPP_KEY As String
  Dim parsedJsonObject As Object
    ...
    'MsgBox "Antes do CarregaAguarde: obj.Open GET, " & url, vbInformation + vbOKOnly, "WhatsAppConectar"

    CarregaAguarde "API WhatsApp: Iniciando conexão..."

    'MsgBox "obj.send " & jsonAPI, vbInformation + vbOKOnly, "WhatsAppConectar"
    obj.send jsonAPI
    Dim resposta As String
    resposta = obj.responseText
    
    'MsgBox "resposta " & resposta, vbInformation + vbOKOnly, "WhatsAppConectar"
    
    Set parsedJsonObject = JSON.parseJSON(resposta)
    
    If obj.Status = 200 Then
        If parsedJsonObject.Item("instance").Item("state") = "open" Or parsedJsonObject.Item("instance").Item("state") = "connecting" Then
           WhatsAppConectar = WhatsAppInstanciaVerificar
           
           url = urlAPIWhatsBase & "instance/connect/" & WPP_KEY
           obj.Open "GET", url
           obj.setRequestHeader "Content-Type", contentType
           obj.setRequestHeader "apikey", API_KEY_WPP
           obj.send jsonAPI
           ...
           If obj.Status = 200 Then
              qrCode = parsedJsonObject.Item("base64")
              If Not Vazio(qrCode) Then qrCode = Mid$(qrCode, InStr(qrCode, "base64,") + 7)
              
              DescarregaAguarde
              
              Load frmQRCode
              If Not Vazio(qrCode) Then frmQRCode.CarregarImagemBase64 (qrCode)
              frmQRCode.Show vbModal
              
              WhatsAppConectar = True
              Exit Function
           ...
           Exit Function
        Else
           GoTo continuaConexao
        End If
    ElseIf obj.Status = 404 Then
continuaConexao:
        url = urlAPIWhatsBase & "instance/delete/" & WPP_KEY
        obj.Open "DELETE", url
        obj.setRequestHeader "Content-Type", contentType
        obj.setRequestHeader "apikey", API_KEY_WPP
        obj.send jsonAPI
        ...
        'MsgBox "resposta " & resposta, vbInformation + vbOKOnly, "WhatsAppConectar: delete"
        
        Set parsedJsonObject = JSON.parseJSON(resposta)
        If obj.Status = 200 Or obj.Status = 404 Then
           url = urlAPIWhatsBase & "instance/create"
           obj.Open "POST", url
           obj.setRequestHeader "Content-Type", contentType
           obj.setRequestHeader "apikey", API_KEY_WPP
           jsonAPI = "{"
           jsonAPI = jsonAPI & """instanceName"":""" & WPP_KEY & ""","
           jsonAPI = jsonAPI & """qrcode"":true" & ","
           jsonAPI = jsonAPI & """integration"":""" & "WHATSAPP-BAILEYS" & """"
           jsonAPI = jsonAPI & "}"
           ...
           If obj.Status = 201 Then
              ComandoSQL = "UPDATE empresa SET WhatsAppApiToken = '" & parsedJsonObject.Item("hash") & "'"
              SQLExecuta ComandoSQL
              
              qrCode = parsedJsonObject.Item("qrcode").Item("base64")
              qrCode = Mid$(qrCode, InStr(qrCode, "base64,") + 7)
              
              DescarregaAguarde
              
              Load frmQRCode
              frmQRCode.CarregarImagemBase64 (qrCode)
              frmQRCode.Show vbModal
              
              WhatsAppConectar = True
              Exit Function
           Else
              CarregaAguarde "API WhatsApp" & vbNewLine & CStr(parsedJsonObject.Item("status")) & " - " & parsedJsonObject.Item("response").Item("message")
              DoEvents
              Sleep 2000
              DescarregaAguarde
              WhatsAppConectar = False
              Exit Function
           End If
        Else
           DescarregaAguarde
    ...
    Else
        DescarregaAguarde
        MsgBox resposta, vbCritical + vbOKOnly, "ATENÇÃO"
        WhatsAppConectar = False
        Exit Function
    End If
...
Resume
deuErro:
    MsgBox Err.Description, vbCritical + vbOKOnly, "ERRO: WhatsAppConectar"
    Err.Clear
    WhatsAppConectar = False
End Function

Public Function WhatsAppDesconectar() As Boolean
  Dim contentType As String, urlBase As String, url As String, ComandoSQL As String, jsonAPI As String, xCNPJCPF As String
  Dim qrCode As String, WPP_KEY As String, processando As String
  Dim parsedJsonObject As Object
    ...
    url = urlAPIWhatsBase & "instance/logout/" & WPP_KEY
    obj.Open "DELETE", url
    obj.setRequestHeader "Content-Type", contentType
    obj.setRequestHeader "apikey", API_KEY_WPP

    CarregaAguarde "API WhatsApp: Desconectando conexão..."
    ...
    'MsgBox "resposta " & resposta, vbInformation + vbOKOnly, "WhatsAppConectar"
    processando = "parseJSON resposta logout"
    Set parsedJsonObject = JSON.parseJSON(resposta)
    
    If obj.Status = 200 Or obj.Status = 404 Then
        If parsedJsonObject.Item("status") = "SUCCESS" Then
           url = urlAPIWhatsBase & "instance/delete/" & WPP_KEY
           obj.Open "DELETE", url
           obj.setRequestHeader "Content-Type", contentType
           obj.setRequestHeader "apikey", API_KEY_WPP
           ...
           processando = "parseJSON resposta delete"
           Set parsedJsonObject = JSON.parseJSON(resposta)
           
           If obj.Status = 200 Then
              If parsedJsonObject.Item("status") = "SUCCESS" Then
                 ComandoSQL = "UPDATE empresa SET WhatsAppApiToken = ''"
                 SQLExecuta ComandoSQL
                 
                 MsgBox "API WhatsApp Desconectada", vbInformation + vbOKOnly, "ATENÇÃO"
                 
                 'Call FormTimeRefresh_F9(False)
                 
                 WhatsAppDesconectar = True
                 DescarregaAguarde
                 Exit Function
              Else
                 DescarregaAguarde
                 DoEvents
                 MsgBox parsedJsonObject.Item("response").Item("message"), vbCritical + vbOKOnly, "ATENÇÃO"
                 WhatsAppDesconectar = False
                 Exit Function
              End If
           Else
              DescarregaAguarde
...
deuErro:
    MsgBox Err.Description + vbNewLine + processando, vbCritical + vbOKOnly, "ERRO: WhatsAppDesconectar"
    Err.Clear
    WhatsAppDesconectar = False
End Function

Public Function WhatsAppInstanciaVerificar() As Boolean
  Dim contentType As String, urlBase As String, url As String, ComandoSQL As String, jsonAPI As String, xCNPJCPF As String
  Dim qrCode As String, Item As Variant, WPP_KEY As String
  Dim parsedJsonObject As Object
    ...
    url = urlAPIWhatsBase & "instance/fetchInstances"
    obj.Open "GET", url
    obj.setRequestHeader "Content-Type", contentType
    obj.setRequestHeader "apikey", API_KEY_WPP
    ...
    If obj.Status = 200 Then
        For Each Item In parsedJsonObject
            If Item("name") = WPP_KEY Then
               If Item("connectionStatus") = "open" Or Item("connectionStatus") = "connecting" Then
                  ComandoSQL = "UPDATE empresa SET WhatsAppApiToken = '" & Item("token") & "'"
                  SQLExecuta ComandoSQL
                  WhatsAppInstanciaVerificar = True
                  DescarregaAguarde
                  Exit For
               End If
            End If
        Next
        WhatsAppInstanciaVerificar = WhatsAppConectado
    ElseIf obj.Status = 404 Then
continuaConexao:
        url = urlAPIWhatsBase & "instance/delete/" & WPP_KEY
        obj.Open "DELETE", url
        ...
        If obj.Status = 200 Or obj.Status = 404 Then
           ...
           If obj.Status = 201 Then
              ...
              If Not Vazio(qrCode) Then MsgBox "instanceId: " & parsedJsonObject.Item("instance").Item("instanceId") & vbNewLine & "status: " & parsedJsonObject.Item("instance").Item("status"), vbInformation + vbOKOnly, "ATENÇÃO"
        ...
        Else
           DescarregaAguarde
           WhatsAppInstanciaVerificar = False
           Exit Function
        End If
    Else
        DescarregaAguarde
...
Public Function WhatsAppMensagemEnvio(Celular As String, Mensagem As String, ByRef mensagemRetorno As String, ByRef mensagemErro As String) As Boolean
  Dim contentType As String, url As String, ComandoSQL As String, jsonAPI As String, xCNPJCPF As String
  Dim parsedJsonObject As Object, WPP_KEY As String
    ...
    obj.Open "POST", urlAPIWhatsBase & "message/sendText/" & WPP_KEY
    obj.setRequestHeader "Content-Type", contentType
    obj.setRequestHeader "apikey", API_KEY_WPP
    
    Mensagem = Replace(Mensagem, vbNewLine, "\n")
    Mensagem = Replace(Mensagem, Chr(13), "\n")
    Mensagem = Replace(Mensagem, Chr(9), "\r")
    Mensagem = Replace(Mensagem, Chr(10), "\r")
    Mensagem = Replace(Mensagem, Chr(11), "\r")
    
    jsonAPI = "{ "
    jsonAPI = jsonAPI & """number"":""" & "55" & Celular & """, "
    jsonAPI = jsonAPI & """text"":""" & Mensagem & """, "
    jsonAPI = jsonAPI & """delay"":" & "5000" & ""
    jsonAPI = jsonAPI & " }"
    
    'MsgBox jsonAPI, vbInformation + vbOKOnly
    
    'fsoSalva "\jsonEnvio.json", jsonAPI, App.Path
    
    'CarregaAguarde "API WhatsApp: Enviando Mensagem..."
    ...
    'MsgBox CStr(obj.Status) & " - " & resposta, vbInformation + vbOKOnly
    
    'fsoSalva "\jsonEnvioResposta.json", resposta, App.Path
    
    DescarregaAguarde
    
    If obj.Status = 201 Then
       mensagemRetorno = parsedJsonObject.Item("key").Item("id") & " - " & parsedJsonObject.Item("status")
    ...
    End If
End Function
Public Function WhatsAppArquivoEnvio(Celular As String, Mensagem As String, Arquivo() As String, ByRef mensagemRetorno As String, ByRef mensagemErro As String) As Boolean
  Dim contentType As String, url As String, Conteudo As String, ComandoSQL As String, jsonAPI As String, xCNPJCPF As String
  Dim i As Integer, nomeArquivo As String, qtArquivos As Integer, WPP_KEY As String
  Dim parsedJsonObject As Object
    ...
    qtArquivos = UBound(Arquivo) - LBound(Arquivo) + 1
    
    For i = LBound(Arquivo) To UBound(Arquivo)
        nomeArquivo = Arquivo(i)
        
        obj.Open "POST", urlAPIWhatsBase & "message/sendMedia/" & WPP_KEY
        obj.setRequestHeader "Content-Type", contentType
        obj.setRequestHeader "apikey", API_KEY_WPP
        
        jsonAPI = "{ "
        jsonAPI = jsonAPI & """number"":""" & "55" & Celular & ""","
        jsonAPI = jsonAPI & """mediatype"":""document"","
        jsonAPI = jsonAPI & """mimeltype"":""application/" & LCase(Right(nomeArquivo, 3)) & ""","
        jsonAPI = jsonAPI & """caption"":""" & EscapeJSON(Dir$(nomeArquivo) & " " & CStr(i + 1) & "/" & CStr(qtArquivos)) & ""","
        jsonAPI = jsonAPI & """media"":""" & EncodeBase64Arquivo(nomeArquivo) & ""","
        jsonAPI = jsonAPI & """fileName"":""" & EscapeJSON(Dir$(nomeArquivo)) & ""","
        jsonAPI = jsonAPI & """delay"":" & "5000" & ""
        jsonAPI = jsonAPI & " }"
        
        'HourglassShow "API WhatsApp: Enviando Arquivo..."
        CarregaAguarde "API WhatsApp: Enviando Arquivo.: " & CStr(i + 1) & "/" & CStr(qtArquivos)
...
Public Sub CarregaAguarde(Mensagem As String)
   'If Not FormEstaAberto("frmaMsgAgua") Then Load frmaMsgAgua
   'frmaMsgAgua.lblMensagem.Caption = Mensagem
   'DoEvents
   'frmaMsgAgua.Show
End Sub

Public Sub DescarregaAguarde()
   'Unload frmaMsgAgua
End Sub

'Salva no arquivo usando os objetos FileSystemObject by Victor
Public Sub fsoSalva(sArquivo As String, sTxt As String, Optional sDir As String, Optional Apendar As Boolean = False)
 Dim fso As Object, fArq As Object, ftxt As Object
 Set fso = CreateObject("Scripting.FileSystemObject")  'Cria o objeto FileSystemObjeto
 If Existe(sDir + sArquivo) Then
   Set fArq = fso.GetFile(sDir + sArquivo)                      'Le o arquivo
   Set ftxt = fArq.OpenAsTextStream(IIf(Apendar, 8, 2), False)            'Abre o arquivo modo gravacao
 Else
   Set ftxt = fso.CreateTextFile(sDir + sArquivo, True)               'Cria o arquivo
 End If
 ftxt.Write sTxt                                       'Grava a string no arquivo
 ftxt.Close                                            'Fecha o arquivo
 'Limpa o objetos da memoria
 Set ftxt = Nothing
 Set fArq = Nothing
 Set fso = Nothing
End Sub

'Abre o arquivo usando os objetos FileSystemObject e retorna o conteúdo do arquivo
Public Function fsoAbre(sArquivo As String, Optional sDir As String) As String
 Dim fso As Object, fArq As Object, ftxt As Object
 If Not Existe(sDir + sArquivo) Then Exit Function
 Set fso = CreateObject("Scripting.FileSystemObject")  'Cria o objeto FileSystemObjeto
 Set fArq = fso.GetFile(sDir + sArquivo)                      'Le o arquivo
 Set ftxt = fArq.OpenAsTextStream(1, False)            'Abre o arquivo modo leitura
 If Not ftxt.AtEndOfStream Then                        'Se nao estiver no final do arquivo
   fsoAbre = ftxt.ReadAll                              'Le tudo para a variavel
 End If
 ftxt.Close                                            'Fecha o arquivo
 'Limpa o objetos da memoria
 Set ftxt = Nothing
