...
'mostrar o ultimo pre�o de compra
...
If cboConsulta.Text = "DETALHADO" Then
    ...
    If cboConsDescricao.Text = "FABRICANTE" Then
        ...
        Do While Not r.EOF
           ...
           r.MoveNext
        Loop
    ElseIf cboConsDescricao.Text = "REFER�NCIA" Then
...
ElseIf cboConsulta.Text = "MENSAL" Then
    ...
    lblConsDescricao.Visible = True
    lblConsDescricao.Caption = "M�s/Ano"
...
ElseIf cboConsulta.Text = "REFER�NCIA" Then
    ...
    cboConsAno.Visible = False
    lblConsDescricao.Visible = True
    lblConsDescricao.Caption = "Refer�ncia"
    cboConsDescricao.Width = 4095
    mskInicial.Visible = False
    mskFinal.Visible = False
    cmdConNotaCal1.Visible = False
...
ElseIf cboConsulta.Text = "C�D. BARRA" Then
    ...
    cboConsAno.Visible = False
    lblConsDescricao.Visible = True
    lblConsDescricao.Caption = "C�d. Barra"
    cboConsDescricao.Width = 4095
    mskInicial.Visible = False
    mskFinal.Visible = False
    cmdConNotaCal1.Visible = False
...
ElseIf cboConsulta.Text = "DETALHADO" Then
    ...
    cboConsAno.Visible = True
    lblConsDescricao.Visible = True
    lblConsDescricao.Caption = "Crit�rio"
    mskInicial.Visible = False
    mskFinal.Visible = False
    cmdConNotaCal1.Visible = False
    cmdConNotaCal2.Visible = False
...
If lblTipoConsulta.Caption = "0" Or lblTipoConsulta.Caption = "2" Then
   ...
   Else
       ShowMsg "Produto n�o cadastrado.", vbExclamation
   ...
   If r.BOF Then ShowMsg "Produto n�o cadastrado.", vbExclamation
   If r.State <> 0 Then r.Close
End If
End Sub
...
Set fCal = Nothing                   'Destr�i a vari�vel
  ...
  Dim sSQL As String
   Dim r As ADODB.Recordset
      ...
      If Not ExistInList(cboConsDescricao) Then
         ShowMsg "Selecione o m�s na lista.", vbExclamation
         Exit Sub
      End If
      ...
      For DIA = dIni To dFim
         ...
         If Not r.BOF Then
            'Atribui os saldo para as vari�veis
...
If txtQuant.Text = "" Or txtQuant.Text = "0" Then
   ShowMsg "Insira uma quantidade v�lida!", vbExclamation
   txtQuant.SetFocus
   Exit Sub
End If

If txtCusto.Text = "" Or txtCusto.Text = "0" Then
   ShowMsg "Insira um custo de produto v�lido!", vbExclamation
   txtCusto.SetFocus
   Exit Sub
End If
...
'If cmdSalvar.Enabled = True Then
   ...
   sSQL = "INSERT INTO produtos_quant (Codigo, COD_PRODUTO, Data, COD_ENTRADA, FORMA, QUANT, TIPO, HORA, COD_USUARIO, ESTOQUE) VALUES (" & _
      AutoNumeracao & ", " & txtCodProduto.Text & ", CONVERT(DATETIME, '" & Format(Date, ocDATA) & "', 103), " & txtCodigo.Text & ", 'ENTRADA', " & Replace(CDbl(txtQuant.Text), ",", ".") & ", 'ADI��O', '" & Format(Now, ocHRMN) & "', " & txtCodUsuario.Text & ", " & Replace(CDbl(txtQuantAtual.Text), ",", ".") & ");"
   dbData.Execute sSQL
'End If
End Sub
...
If txtCodigo.Text = "" Then
   MsgBox "FORMUL�RIO INCOMPLETO!" & vbCrLf & "Consulte a NOTA FISCAL na guia CONSULTA.", vbInformation, "Aviso do Sistema"
   Exit Sub
End If

'N�o � necess�rio consulta o registro antes de atualiza-lo
sSQL = "SELECT * FROM produtos_entrada WHERE (codigo = " & txtCodigo.Text & ");"
Set r = dbData.OpenRecordset(sSQL)

If Not Atualizar_Dados Then
   ShowMsg "N�o foi poss�vel atualizar o registro." & vbCr & "Verifique os dados informados e tente novamente.", vbExclamation
   Exit Sub
End If
...
Private Sub cmdCadProdutos_Click()
   ...
   'Substituiu a abertura da tabela de configura��o
...
If cmdAlterar.Enabled = True Then
    MsgBox "Entrada j� gravada n�o pode ser cancelada!", vbInformation, "Aviso do Sistema"
Else
...
'If Tela_Principal.txtNivel.Text <> "1" Then MsgBox "Seu n�vel de acesso n�o permite a essa opera��o!", vbInformation, "Aviso do Sistema": Exit Sub

If txtCodigo.Text = "" Then
   ShowMsg "FORMUL�RIO INCOMPLETO!" & vbCrLf & "Consulte Nota Fiscal na guia CONSULTA", vbInformation
   Exit Sub
End If
...
'===FUN��O DE CONTAR REGISTROS
lblQuant.Caption = Format(totalRegistros, "00")

If cboConsulta.Text = "TODOS" Or cboConsulta.Text = "NOTA FISCAL" Or cboConsulta.Text = "FORNECEDOR" Or cboConsulta.Text = "MENSAL" Or cboConsulta.Text = "PER�ODO" Then
    FormatarGrid r
Else
...
Private Sub cmdFechar_Click()
   If txtCodigo.Text <> "" And cmdSalvar.Enabled = True Then
      ShowMsg "ENTRADA EM ABERTO!" & vbCrLf & "Clique no bot�o SALVAR ou no CANCELAR.", vbInformation
      Exit Sub
   Else
      Unload Me
   End If
...
If cboConsulta.Text = "TODOS" Or cboConsulta.Text = "NOTA FISCAL" Or cboConsulta.Text = "FORNECEDOR" Or cboConsulta.Text = "MENSAL" Or cboConsulta.Text = "PER�ODO" Then
   ...
   If cboConsulta.Text = "MENSAL" Then
      REL_Prod_Entrada_Nota.dfTipo.Caption = "Tipo: M�s = " & cboConsDescricao.Text & "/" & cboConsAno.Text
   ElseIf cboConsulta.Text = "PER�ODO" Then
      REL_Prod_Entrada_Nota.dfTipo.Caption = "Per�odo: " & mskInicial.Text & " at� " & mskFinal.Text
   ElseIf cboConsulta.Text = "FORNECEDOR" Then
      REL_Prod_Entrada_Nota.dfTipo.Caption = "Tipo: Fornecedor = " & cboConsDescricao.Text & ""
   ElseIf cboConsulta.Text = "NOTA FISCAL" Then
      REL_Prod_Entrada_Nota.dfTipo.Caption = "Tipo: Nota Fiscal N� " & cboConsDescricao.Text & ""
   Else
...
Else
   ...
   ElseIf cboConsulta.Text = "REFER�NCIA" Then
      REL_Prod_Entrada_PorProduto.dfTipo.Caption = "Tipo: Refer�ncia " & cboConsDescricao.Text & ""
   ElseIf cboConsulta.Text = "C�D. BARRA" Then
      REL_Prod_Entrada_PorProduto.dfTipo.Caption = "Tipo: C�d. de Barra " & cboConsDescricao.Text & ""
   End If
...
If Not Inserir_Dados Then
   ShowMsg "N�o foi poss�vel cadastrar o registro." & vbCr & "Verifique os dados informados e tente novamente.", vbExclamation
   Exit Sub
End If
...
'Retorna o resultado da atualiza��o
...
With Grid
   ...
   'colocar os cabe�alho em negrito
   For x = 0 To .Cols - 1
      .Col = x
      .Row = 0
      .CellFontBold = True
...
'CHECAR SE O PEDIDO EST� FECHADO
If txtCodigo.Text = "" Then Exit Sub
...
If Grid.TextMatrix(Grid.Row, 8) = "XML" Then
    MsgBox "N�o � permitida a visualiza��o de uma nota fiscal com tipo de entrada igual a XML por aqui!", vbInformation, "Aviso do Sistema"
    Exit Sub
End If
...
'CALCULAR PRE�O - ATACADO A VISTA
...
'CALCULAR PRE�O - ATACADO A PRAZO
Dim varMargemAP As Currency
