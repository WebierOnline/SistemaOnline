...
If KeyCode = 13 Then
   ...
   If VERIFICAR_QUANTIDADE = True Then
      ...
      Exit Sub
   Else
      OS_Informatica.txtCodPeca.Text = Grid.TextMatrix(Grid.Row, 1)
      OS_Informatica.txtPecas.Text = Grid.TextMatrix(Grid.Row, 3)
      OS_Informatica.mskValorPeca.Text = Grid.TextMatrix(Grid.Row, 9)
      Unload Me
      On Local Error Resume Next
      OS_Informatica.txtQuantPeca.SetFocus
   End If
End If
End Sub
...
'            .TextMatrix(.Rows - 1, 4) = rTabela("var_fab")
            
            .TextMatrix(.Rows - 1, 6) = rTabela("var_med")
'            .TextMatrix(.Rows - 1, 7) = rTabela("var_prat")
