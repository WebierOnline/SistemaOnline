...
Begin VB.Form Frm_ItemNFCe 
   BackColor       =   &H00D8E9EC&
   BorderStyle     =   3  'Fixed Dialog
   Caption         =   "Itens da NFCe"
   ClientHeight    =   5775
   ClientLeft      =   1605
   ClientTop       =   2280
   ClientWidth     =   9480
   ...
   End
End
Attribute VB_Name = "Frm_ItemNFCe"
...
Dim sSQL As String, seq As Integer
    sSQL = "SELECT MAX(Item) r FROM NotaFiscalItens WHERE CodigoNota = " & Val(Frm_NFCe.txtCodNota.Text)
    seq = SQLExecutaRetorno(sSQL, "r", 0) + 1
    Tb("CodigoNota") = Format(Frm_NFCe.txtCodNota.Text, "@")
    Tb("Item") = seq
...
Dim totalRegistros As Long
    ...
    sSQL = "SELECT * FROM NotaFiscalItens WHERE CodigoNota = " & Val(Frm_NFCe.txtCodNota.Text)
    RsOpen Tb, sSQL
    
    If Tb.RecordCount > 0 Then totalRegistros = Tb.RecordCount
    
    txtCodNota.Text = Frm_NFCe.txtCodNota.Text
    
    enviada = SQLExecutaRetorno("SELECT Enviada FROM NotaFiscal WHERE CodigoNota = " & Val(Frm_NFCe.txtCodNota.Text), "Enviada", 0)
...
Private Sub Form_Unload(Cancel As Integer)
   RsOpen Frm_NFCe.TbNotas, "SELECT *, " & _
                    ...
                    "FROM NotaFiscal"
    'Frm_NFCe.FormatarGridItensNota Frm_NFCe.TbNotas  'desativei ver depois
End Sub
...
If KeyCode = 13 Then
    ...
    vgDb.CommitTrans
    Limpa_Tudo Me
    sSQL = "SELECT ISNULL(SUM(ValorTotalBruto), 0) r FROM NotaFiscalItens WHERE CodigoNota = " & Val(Frm_NFCe.txtCodNota.Text)
    vTotal = SQLExecutaRetorno(sSQL, "r", 0)
    sSQL = "UPDATE NotaFiscal SET ValorProdutos = " & FSQL(vTotal, 2) & ", ValorNota = " & FSQL(vTotal, 2) & " WHERE CodigoNota = " & Val(Frm_NFCe.txtCodNota.Text)
    SQLExecuta sSQL
    sSQL = "SELECT * FROM NotaFiscalItens WHERE CodigoNota = " & Val(Frm_NFCe.txtCodNota.Text)
    RsOpen Tb, sSQL
    FormatarGridItensNota Tb
