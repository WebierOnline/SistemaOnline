-- Acrescenta ValorTributos (item TbNFCe_Itens) na NFCeIncluir, calculado via tbNCM (tabela
-- do IBPT, sincronizada por frmImportarIBPT.frm em OnlineCommerce). Sem isso, o campo ficava
-- sempre 0/NULL e o DANFE da NFCe imprimia "Informacao dos Tributos Totais Incidentes
-- (Lei Federal 12.741/2012) 0,00" - o valor e somado item a item em TransmitirNFCe
-- (modNFe.bas) e repassado como vTotTrib pro GerarTotalProdutos, mas so funciona se o
-- campo do item estiver preenchido.
-- Formula padrao Lei 12.741/2012: valor do item x (federal + estadual + municipal) / 100,
-- usando SEMPRE nacionalfederal (nao importadosfederal - loja nao rastreia origem por item).
-- LEFT JOIN (nao INNER) pra nao quebrar a venda se o produto tiver um NCM fora da tabela IBPT
-- (fica ValorTributos = 0 nesse caso, igual ao comportamento anterior pra aquele item).
